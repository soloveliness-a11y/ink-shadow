...
npx tsx packages/visual-pipeline/src/cli.ts content/_mock/script.json \
  --api-key "sk-xxxx" \
  --interval 240
...
# 只看任务清单不出图
npx tsx packages/visual-pipeline/src/cli.ts content/_mock/script.json --dry-run
...
npx tsx packages/visual-pipeline/src/cli.ts content/_mock/script.json --model stub
```
...
# 看生成了几张
ls content/_mock/assets/*.png | wc -l

# 看 script.json 里 asset 是否回填
jq '.characters[0].visual.asset.status' content/_mock/script.json  # 应该是 "done"

# 看剧本状态
jq '.meta.status' content/_mock/script.json  # 全部完成后应该是 "ready"
```
